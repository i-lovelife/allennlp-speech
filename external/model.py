...
if __name__ == '__main__':
    ...
    speaker_classifier = SpeakerClassifier(n_class=2).cuda()
    #S = SpeakerClassifier().cuda()
    #cbhg = CBHG().cuda()
    inp = Variable(torch.randn(16, 513, 128)).cuda()
    e1 = E1(inp)
    print('output shape:')
    print(f'encoder: {e1.size()}')
    out_classifier = speaker_classifier(e1)
    print(f'speaker_classifier: {out_classifier.size()}')
    
    """
    #s1 = S(e1)
    c = Variable(torch.from_numpy(np.random.randint(8, size=(16)))).cuda()
    d = D(e1, c)
    print(f'decoder: {d.size()}')
